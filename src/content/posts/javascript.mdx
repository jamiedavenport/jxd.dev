...
The `in` operator can be used to check if a property exists on an object. `for ... in` can be used to iterate over all keys in an object. Number keys are ordered.
...
> Note that function properties are not supported by `structuredClone()`. Trying to clone an object with a function property will throw an error.

## `this` keyword

The `this` keyword is bound to the object that the function is called on. It does not exist when using arrow functions.
